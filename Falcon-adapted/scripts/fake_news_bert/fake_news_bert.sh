...
OUTDIR="/cluster/work/Falcon-results/$1/$2/$(date +%F_%H-%M-%S)"
...
if [ ! -f "$NEIGHBOR_FILE" ]; then
  ...
  python find_neighbors.py \
    --cfg_file configs/fakenews/coarse2fine/base.yaml \
...
python infer_fine_labels.py \
  --cfg_file configs/fakenews/coarse2fine/base.yaml \
  --override_cfg DATASET.NAME FakeNews DATASET.DATAROOT "$INFILE" \
