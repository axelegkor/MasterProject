...
def warmstart_assignment_matrix(labels_fine, labels_coarse, num_fine, num_coarse):
    M = torch.zeros(num_fine, num_coarse)
    for fine, coarse in zip(labels_fine, labels_coarse):
        M[fine, coarse] = 1
    # Avoid zero-only rows
    M[M.sum(dim=1) == 0] = 1
    return M
...
def worker(device_id, cfg):
    logger = (
        Logger(filename=os.path.join(cfg.OUTPUT_DIR, "log.txt"))
        if device_id == 0
        else None
    )
    (
    ...
    train_loader, val_loader, test_loader, fine_classes, coarse_classes = (
        construct_coarse2fine_loader(cfg)
    )
    print(fine_classes, coarse_classes)
    if cfg.MODEL.NUM_CLASSES is None:
        cfg.MODEL.NUM_CLASSES = fine_classes

    model, embed_dim = create_backbone_factory(cfg)(cfg.MODEL.PRETRAINED)
    model = construct_classifier(
        cfg.MODEL.HEAD_TYPE, model, embed_dim, fine_classes
    ).cuda(device_id)
    if cfg.MODEL.FROZEN:
    ...
    dataset = train_loader.dataset.dataset  # Get FakeNewsBertData
    labels_fine = []
    labels_coarse = []
    for i in range(len(dataset)):
        sample = dataset[i]
        labels_fine.append(sample["fine_label"])
        labels_coarse.append(sample["coarse_label"])
    labels_fine = torch.tensor(labels_fine)
    labels_coarse = torch.tensor(labels_coarse)

    M = warmstart_assignment_matrix(
        labels_fine, labels_coarse, fine_classes, coarse_classes
    ).cuda()
    ...
    for epoch in range(start_epoch, cfg.SOLVER.MAX_EPOCHS + 1):
        tau = (
            cosine_annealing(1, 0.0, epoch - 1, cfg.SOLVER.SOFT_LABELS_EPOCHS)
            if epoch <= cfg.SOLVER.SOFT_LABELS_EPOCHS
            else 0.0
        )
        cfg.SOLVER.DISCRETE_OPTIM.BETA_REG = (
            cfg.SOLVER.DISCRETE_OPTIM.BETA_REG
            if epoch <= cfg.SOLVER.SOFT_LABELS_EPOCHS
            else 0.0
        )
        ...
        t0 = time_ns()
        M = train_one_epoch(
            cfg, model, M, optimizer, train_loader, scaler, ema, tau=tau
        )
        t1 = time_ns()
        ...
        if scheduler is not None:
            scheduler.step()
            (
                logger.log(f"Learning rate model: {scheduler.get_last_lr()}")
                if device_id == 0
                else None
            )
        ...
        (
            make_snapshot(cfg, epoch, model_without_ddp, M, ema, optimizer, scheduler)
            if device_id == 0
            else None
        )
