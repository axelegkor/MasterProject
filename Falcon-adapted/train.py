...
def train_one_epoch(cfg, model, M, optimizer, train_loader, scaler, ema, tau):
    ...
    print(f"✅ Model output dim (should be 7): {cfg.MODEL.NUM_CLASSES}")
    ...
    for i, data in enumerate(train_loader):
        ...
        with torch.cuda.amp.autocast(enabled=cfg.MODEL.PRECISION == "fp16"):
            with torch.no_grad():
                with ema.average_parameters():
                    ...
                    neighbors = rearrange(
                        model(neighbors.cuda()).softmax(-1).detach(),
                        "(n k) c -> n k c",
                        n=N,
                        k=num_neig,
                    )
            ...
            logits = model(x)
            print("🧠 Predicted fine label classes in batch:", logits.argmax(dim=1).unique().cpu().tolist())
            probs = logits.softmax(-1)

            log_coarse_prob = torch.logsumexp(
                logits.unsqueeze(-1)
                .repeat(1, 1, M.shape[1])
                .masked_fill_(
                    ~M.unsqueeze(0).repeat(logits.shape[0], 1, 1).bool(), float("-inf")
                ),
                dim=1,
            ...
            q_fine_soft = (
                (logits_ema / cfg.SOLVER.LOSS.TEMP)
            ...
            loss_total = (
                cfg.SOLVER.LOSS.LAMBDA_1 * loss_cls_coarse
        ...
        if iters % 10 == 0:
            print(
                f"Total loss: {total_loss/iters:.4f} Cls Fine: {cls_loss_fine / iters:.4f}, Cls Coarse: {cls_loss_coarse / iters:.4f}, Reg: {reg_loss / iters:.4f}"
            )
        ...
        if i % cfg.SOLVER.DISCRETE_OPTIM.SOLVE_EVERY == 0 and i > 0:
            ...
            M = compute_assignment_from_cost(
                cost.cpu().numpy(),
                reg_coef=cfg.SOLVER.DISCRETE_OPTIM.BETA_REG,
                time_limit=cfg.SOLVER.DISCRETE_OPTIM.TIME_LIMIT,
            ).cuda()
            dist.broadcast(M, 0)
